...
import WebKit
import CoreLocation
class SetPasswordViewController: UIViewController,WKNavigationDelegate{
    ...
    override func viewDidLoad() {
        ...
        if let dictInfo_ = UserDefaults.standard.value(forKey: "SaveDict") as?  [String : String] {
            let url = URL(string: "http://www.incabdispatch.com/justDriverLogin.aspx?dot=\(dictInfo_["companyDot"] ?? "0")&driverid=\(dictInfo_["driverID"] ?? "0")&deviceid=\(dictInfo_["deviceID"] ?? "0")&truckid=\(dictInfo_["truckID"] ?? "0")&trailerid=\(dictInfo_["TrailerID"] ?? "0")")!
...
//        var apiURL = "http://www.incabdispatch.com/justSendAddress.aspx?dot=2150093&timestamp=\(todayDate)&PhoneNumber=4174553609&drivertype=Driver&driver=1&truck=1&orderid=&lat=36.7314857&lon=-94.41146345&distance=14.028028689250945&speed=0.0&timespan=14015&address=&onJob=&trailer=2&manufacturer=LGE&model=LG-US996&version=3.0"
      ...
      var apiURL = ""
       ...
       if let dictValue = UserDefaults.standard.value(forKey: "SaveDict") as? [String : String] {
          apiURL = "http://www.incabdispatch.com/justSendAddress.aspx?dot=\(dictValue["companyDot"] ?? "")&timestamp=\(todayDate)&PhoneNumber=4174553609&drivertype=Driver&driver=\(dictValue["driverID"] ?? "")&truck=\(dictValue["truckID"] ?? "")&orderid=&lat=\(lat)&lon=\(long)&distance=0.14374149186357857&speed=0.0&timespan=15631&address=&onJob=&trailer=\(dictValue["TrailerID"] ?? "")&manufacturer=ios&model=iphone&version=10.3"
      }
