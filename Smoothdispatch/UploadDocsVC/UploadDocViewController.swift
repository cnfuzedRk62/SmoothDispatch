...
class UploadDocViewController: UIViewController,ShowAlert{
    ...
    override func viewDidLoad() {
        ...
        self.registerXib()
        initDict()
        url = "http://www.incabdispatch.com/justcatchimage.aspx?dot=2150093&deviceid=4174553609&ordernum=1&driverid=1&truckid=1&trailerid=1&drivertype=driver&note=test&doctype=MISC"
...
//        cell.imageView_.image = image
        ...
        if let dictInfo_ = UserDefaults.standard.value(forKey: "SaveDict") as?  [String : String] {
           ...
           let Url =    "http://www.incabdispatch.com/justcatchimage.aspx?dot=\(dictInfo_["companyDot"] ?? "0")&deviceid=\(dictInfo_["deviceID"] ?? "0")&ordernum=\(DictData["ordernum"] as? String ?? "")&driverid=\(dictInfo_["driverID"] ?? "0")&truckid=\(dictInfo_["truckID"] ?? "0")&trailerid=\(dictInfo_["TrailerID"] ?? "0")&drivertype=driver&note=\(DictData["note"] as? String ?? "")&doctype=\(DictData["doctype"] as? String ?? "")"
